...
@pytest.mark.parametrize(('fieldname', 'color_code', 'field_id', 'row_action', 'message'), (
        # 学習分野の重複エラー
        ('Docker', '#000000', '', 'new', 'Dockerは既に登録されています'),
        # 登録（成功）
...
def test_study_fields_process(app, register_user, auth_client, register_fields, fieldname, color_code, field_id, row_action, message):
    with app.test_request_context():
        url = url_for('study.study_fields_process', user_id=register_user['user_id'])
    form_data = {'fieldname[]': fieldname, 'color_code[]': color_code, 'field_id[]': field_id, 'row_action[]': row_action}
